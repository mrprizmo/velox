...
class VectorFuzzer {
 public:
  ...
  std::vector<vector_size_t> randomPartition(vector_size_t size, size_t k);

  void fuzzOffsetsAndTags(
      BufferPtr& offsets,
      BufferPtr& tags,
      const std::vector<vector_size_t>& subSizes);

  UnionVectorPtr fuzzUnion(const UnionTypePtr& unionType);

  UnionVectorPtr fuzzUnion(
  ...
  UnionVectorPtr fuzzUnion(std::vector<VectorPtr>&& children);
  ...
  UnionTypePtr randUnionType(int maxDepth = 5);
...
UnionTypePtr randUnionType(FuzzerGenerator& rng, int maxDepth = 5);
